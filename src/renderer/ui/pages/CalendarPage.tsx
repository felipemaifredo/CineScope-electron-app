...
//Types
import { type TMDBSeries, type TMDBEpisode } from "../../Lib/utils/tmdb"
...
export const CalendarPage = () => {
  ...
  useEffect(function () {
    async function fetchEpisodes() {
      ...
      try {
        const promises = watchlist.map(async function (series) {
  ...
  const selectedDateEvents = selectedDate ? events.filter(function (e) {
      ...
      cells.push(
        ...
        >
          ...
          <div className="mt-2 space-y-1">
            {dayEvents.slice(0, 3).map(function (evt, i) {
              return (
    ...
    <div className="h-full flex flex-col space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4 bg-zinc-900 rounded-lg p-1 border border-zinc-800">
          <button onClick={function () { changeMonth(-1) }} className="p-2 hover:bg-zinc-800 rounded-md transition-colors">
            <ChevronLeft size={20} />
          </button>
      ...
      >
        <div className="space-y-4">
          {selectedDateEvents.length === 0 ? (
            ...
            selectedDateEvents.map(function (evt, i) {
              return (
